...
Usage:
    ...
    6. To encrypt a whole word: type it in the "Word to encrypt" box and
       ...
       for that character's own reply frame before sending the next. Each
       character's full ciphertext (u[], v[], and OUT) appears live in the
       "Per-character ciphertext" log as it comes back, and the decoded
       bytes are reassembled into the final recovered word below it.
...
def encrypt_word():
    ...
    word_log.configure(state="normal")
    word_log.delete("1.0", tk.END)
    word_log.configure(state="disabled")
...
def _encrypt_word_worker(word):
    ...
    for idx, ch in enumerate(word):
        ...
        done_count = idx + 1
        match_tag = "OK" if out == byte else "MISMATCH"
        log_line = (f"{ch!r:>4} (byte={byte:>3})  "
                    f"U={u}  V={v}  OUT={out:>3} [{match_tag}]\n")

        def append_log(line=log_line, dc=done_count, n=len(word)):
            word_log.configure(state="normal")
            word_log.insert(tk.END, line)
            word_log.see(tk.END)
            word_log.configure(state="disabled")
            word_status.set(f"Encrypting... {dc}/{n} characters")
...
tk.Label(root, textvariable=word_result, font=("Consolas", 11, "bold"), wraplength=520).pack(pady=(0, 5))

ttk.Label(root, text="Per-character ciphertext:").pack(anchor="w", padx=15)
word_log = scrolledtext.ScrolledText(root, height=8, width=68, font=("Consolas", 9), state="disabled")
word_log.pack(padx=15, pady=(0, 10))
